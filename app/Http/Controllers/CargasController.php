...
use App\Models\ParadaProgramacao;
...
class CargasController extends Controller
{
    public function store(Request $request, Viagem $viagem)
    {
        $dados = $request->validate([
            ...
            'parada_programacao_id'     => 'nullable|exists:paradas_programacao,id',
        ]);

        $paradaProgramacao = null;
        if (! empty($dados['parada_programacao_id'])) {
            // Só aceita a sugestão se pertencer à programação que gerou esta
            // viagem — evita marcar como "convertida" uma parada de outra
            // viagem/empresa só porque o id foi adivinhado na requisição.
            $paradaProgramacao = ParadaProgramacao::where('id', $dados['parada_programacao_id'])
                ->where('programacao_viagem_id', $viagem->programacao?->id)
                ->whereNull('carga_id')
                ->first();
        }
        unset($dados['parada_programacao_id']);
        ...
        $paradaProgramacao?->forceFill(['carga_id' => $carga->id])->save();
