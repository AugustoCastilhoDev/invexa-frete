...
- **Ainda TBD, só confirmável no primeiro teste real** (Focus NFe não contratada ainda): schema exato do `POST /v2/empresas` (nomes de campo do certificado) e o nome do campo de referência no payload do webhook — ambos marcados no código como melhor palpite contra a doc, nunca validados numa chamada real. Se a Focus exigir cada CNPJ (matriz e cada filial) registrado como sub-empresa própria na conta deles, `Unidade` provavelmente vai precisar do seu próprio `focus_nfe_empresa_id`/token mais adiante — hoje a autenticação com a Focus continua só na `Empresa`, só o CNPJ dentro do payload muda por unidade
