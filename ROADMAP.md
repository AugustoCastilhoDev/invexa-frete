...
- **Restauração testada de ponta a ponta em 2026-07-30**: rodei `backup:run` de verdade (disco local), abri o zip gerado, restaurei o dump SQL num banco isolado (`invexa_frete_restore_test`) e comparei contagem de linhas + conteúdo de todas as tabelas principais (`empresas`, `users`, `viagens`, `clientes`, `motoristas`, `veiculos`, `documentos`, `logs_acesso`) contra o banco original — bateu exatamente em todas, sem nenhuma linha perdida ou divergente. Banco de teste e arquivo de backup local foram removidos depois do teste. **O que esse teste NÃO cobriu**: a cópia na Cloudflare R2 (testei só o disco local, de propósito, pra não gerar tráfego/custo real no bucket de produção a partir de um ambiente de dev) e a disponibilidade do `mysqldump` no VPS de produção em si (testado só no Windows local, onde o binário nem estava no PATH por padrão — teve que ser localizado manualmente). Antes do primeiro cliente pagante, vale repetir este mesmo teste rodando direto no VPS.
...
- **Teste de carga e concorrência em produção (2026-07-18)**: leitura simultânea estável até ~450 requisições sem erro na tela mais pesada do painel (Dashboard), primeiros sinais de fila só perto de 600, sem nenhum erro; escrita simultânea (lançamentos na mesma viagem, importações CSV na mesma empresa) sem perda de dado nos cenários testados. Número específico da VPS atual (plano Hostinger KVM 1) — revisitar se o plano mudar ou o volume de clientes crescer muito. [Relatório completo](https://claude.ai/code/artifact/ae23fae1-d5b5-43f6-a8a4-e6ad9fe81ad2) (privado — liberar visualização caso a caso)
...
- **CIOT**: não implementado. Desde 24/05/2026 o CIOT passou a ser obrigatório pra toda operação de transporte rodoviário remunerado de cargas, inclusive frota própria (antes a premissa era "só TAC/autônomo" — desatualizada), com vínculo regulatório entre CIOT e MDF-e. Bloqueado aguardando confirmação da Focus NFe sobre suporte ao modelo pós-24/05 (chamado de suporte enviado, resposta pendente) — ver `[[ciot_regulacao_2026]]` na memória do projeto. Sem CIOT, o produto não deve ser comunicado como tendo jornada fiscal completa
- **Contingência fiscal** (CT-e/MDF-e): não implementado. MDF-e só tem contingência Offline (motorista com DAMDFE impresso sem internet — não se aplica a um app web operado do escritório); CT-e tem SVC/EPEC, provavelmente tratado de forma transparente pela própria Focus NFe como fornecedora full-service, mas isso não está confirmado — mesma pendência de confirmar com o suporte da Focus que o CIOT. Escopo real (se é preciso codar algo no Invexa Frete ou não) só fica claro depois dessa resposta
...
- **Piso mínimo de frete (ANTT)**: não implementado, e ainda não avaliado a fundo — não há hoje nenhuma validação ou referência a piso mínimo em lugar nenhum do sistema. Identificado num dossiê externo de avaliação de prontidão pra POC (2026-07-30) como requisito regulatório que precisa de uma decisão explícita: onde se aplica, como validar e o que fazer numa rejeição — ainda não escopado
- **Ambiente de homologação/staging + pipeline de deploy com rollback**: hoje o deploy é manual via SSH direto na VPS de produção (ver `[[production_deploy]]`), sem ambiente intermediário nem rollback automatizado — o `homologacao`/`producao` que existe hoje é só do ambiente Focus NFe, por empresa, não do próprio Invexa Frete. Mesmo dossiê de 2026-07-30 apontou isso como lacuna crítica de engenharia. Decisão consciente de **não mexer agora**: envolve custo real (nova VPS ou reconfiguração da atual) e mudança de processo — fica registrado como pendência até haver decisão de investir nisso
