    ...
    <div class="card-body p-4">
        <form action="{{ route('veiculos.store') }}" method="POST">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4 {{ old('tipo') !== 'carreta' ? 'd-none' : '' }}" id="wrapper-tipo-carroceria">
                    ...
                    <select name="tipo_carroceria" class="form-select">
                        ...
                        @foreach(\App\Models\Veiculo::TIPOS_CARROCERIA as $val => $label)
                            <option value="{{ $val }}" {{ old('tipo_carroceria') == $val ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
