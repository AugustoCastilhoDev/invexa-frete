...
return [
    ...
    'backup' => [
        ...
        'source' => [
            'files' => [
                 ...
                 */
                /*
                 * O código-fonte já vive no git; não faz sentido (e é um risco de
                 * segurança desnecessário) incluir o projeto inteiro — incluindo o
                 * .env com as credenciais — dentro do backup. Guardamos só os
                 * arquivos locais que não estão no git (uploads, quando o disco
                 * de uploads é "public"/local em vez de R2).
                 */
                'include' => [
        ...
        'destination' => [
             ...
             */
            'filename_prefix' => 'invexa-frete-',

            /*
             * "local" fica no próprio disco do VPS (acesso rápido pra restore de
             * emergência); "r2" reaproveita o mesmo disco S3-compatível já usado
             * pelos uploads (comprovantes/documentos), como cópia fora do servidor —
             * se o VPS falhar por completo, o backup sobrevive.
             */
            'disks' => [
                'local',
                'r2',
    ...
    'notifications' => [
        ...
        'mail' => [
            'to' => env('BACKUP_NOTIFICATION_EMAIL', 'contato@invexa-app.com.br'),
    ...
    'monitor_backups' => [
        [
            ...
            'disks' => ['local', 'r2'],
            'health_checks' => [
                // 2 dias de folga sobre a cadência diária, pra não disparar falso alarme
                // se o backup atrasar um pouco.
